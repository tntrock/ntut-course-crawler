...
| `capacity.json` | 教室容量（座位數） | 約 52 KB |
...
- **`at` 是偵測到的時間，不是學校異動的時間** —— 實際異動發生在上一次抓取
  與這次之間，最多相差 4 小時。
- **`course_removed` 是跟學校查證過的。** 課從資料集消失有兩種可能：真的停開，
  或者我們那一輪讀錯了。只有「消失」還不夠 —— 每一筆都會再去問學校的教學大綱頁，
  它回「查無課號」才記成停開。問不出結論的那一輪整個不發布，寧可晚 4 小時，
  也不要在這裡放一筆假的停開。
...
> `semesters[].fetched` 是**進度的分子**：現在還有大綱連結、而且已經抓過的
> 門數。它跟頂層 `fetched` 的筆數不一定相等——課換了老師，學校的大綱連結
> 會跟著消失，但抓過的那筆逐課狀態刻意留著（連結回來時不必重抓）。
> 分母 `with_url` 是當下有連結的門數，所以進度不會超過 100%。
...
**約半數教室沒有座位數。** 全量實測 445 間裡有 216 間查得到（48.5%），
其餘 229 間學校根本沒登記 —— 那格連同旁邊的使用率欄位一起是空白的，
打四個不同學年期（115-1 / 110-1 / 105-1 / 199-1）都一樣。這種 `null` 是
永久且正確的，不是抓取失敗。

分佈不是隨機的，一般教學大樓幾乎都有、研究與系館幾乎都沒有：
...
已知容量的 216 間範圍是 10～420，中位數 50。

> **「原本有值、這次讀不到」會保留舊值。** 座位數不會無聲消失 —— 改建
> 會改變數字，不會把數字變成空白。所以那種情況一律保留舊值、更新
> `checked_at`、並在 `errors.json` 記一筆；比例超過門檻時整批不發布，
> 讓 workflow 重試。這是為了擋掉「學校改版讓 216 間同時歸零」這種
> 靠事後看資料分辨不出來的災難。
...
| `capacity` | 每月一次 | 1 號 10 時(排在同一天的整包重建之前) |
...
| `--with-capacity` | 順便抓教室容量（座位數），每月重抓一輪，預設關閉 |
| `--max-capacity N` | 這次最多抓幾間教室，預設 `0` = 不限（全部約 445 間、9 分鐘） |
| `--capacity-refresh-after H` | 同一間教室的容量隔多久重抓，預設 `480` 小時（20 天） |
...
`backfill.yml` 也有這一層（3 次 × 30 分鐘）。它本來就是可重入的 —— 手動觸發、
抓過的學期會永久跳過、重跑一次就從斷掉的地方接續 —— 但補大綱一批要兩個多
小時，中間遇到學校斷線就整批白跑，所以還是值得等一下再試。
...
抓取的容錯分成四層，**每一層失敗都不會賠掉其他層已經完成的成果**：
...
失敗的學期**刻意不寫進 `meta.json`** —— 在那裡留下紀錄會讓下次執行以為
這學期抓過了，於是永遠不會重試。`errors.json` 是「這次發生什麼事」，
`meta.json` 是「我手上有什麼資料」，兩者不能混。

唯一不落地的情況是**斷路器在某個學期抓到一半跳開**：那些還沒問到的系所的
「0 門課」是放棄去問、不是真的沒開課，寫出去會蓋掉線上完整的資料。
這種情況整個學期作廢往上拋，下次重來。

#### 發布也是失敗照發

三支 workflow 的 `Publish to gh-pages` 都設了 `always()`：**抓到一半失敗、
或 job 逾時被砍掉，也要把已經完成的部分推上去。** 它們都是 `keep_files`，
只會補上 `data/` 裡有的檔案，沒寫出來的維持遠端原樣，所以推「半批成果」
不會傷到任何東西 —— 而不推的話，跑了好幾個小時的成果就只剩 artifact 裡
那一份，要靠人工撈回來。

前提是磁碟上不能有寫到一半的 JSON。所以 `_write_json()` 是**先寫 `.tmp`
再 `os.replace()`** —— rename 在同一個檔案系統上是原子的，任何時刻檔案
要嘛是完整的舊版、要嘛是完整的新版。`index.json` 有 1.9 MB，job 逾時是
直接砍行程的，寫到一半被砍的機率不算低，而一份被截斷的 `index.json`
推上線就是整個 API 掛掉，比少更新一輪嚴重得多。

> **`rebuild` 那一支例外，刻意沒有 `always()`。** 它用 `force_orphan`，
> 推上去會把整個分支覆蓋掉；抓失敗時發布等於拿半套資料去蓋掉全部。

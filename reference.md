...
現在真的會照排程跑的只有三支（`backfill` 只剩手動）：

| workflow | cron | 一天幾次 | 請求/次 |
|---|---|---|---|
| `crawl` | `0 */4 * * *` | 6 | 約 355 |
| `syllabus` | `0 1` / `0 13` | 2 | 約 2,270（含約 1,900 份大綱） |
| `crawl`（rebuild） | `0 6 1 * *` | 每月 | 約 355 |
| `capacity` | `0 2 1 * *` | 每月 | 約 580 |

⚠️ **每月 1 號那兩支排在同一天**：`capacity` 02:00（timeout 180 分）與 rebuild
06:00。cron 實測常延遲 2~4 小時，兩者有機會撞上；`cancel-in-progress: false`
所以不會互砍，但後到的會排隊、再後到的會把它擠掉。兩支都還沒被實際觸發過
（見第 7 節），10/1 那天值得看一眼，真撞到就把 `capacity` 挪到別天。

當期課表一天被完整抓 **8 次**（`crawl` 6 + `syllabus` 2）。`syllabus` 那兩趟
的課表**不是白抓**：`--with-syllabus` 需要每門課的 `syllabus_url`，而那只有
課表頁上有（`index.json` 裡沒有），而且那兩趟也順便更新了課表與人數。
`capacity` 的課表抓取同理，理由寫在該 workflow 的註解裡。
...
大綱回補：**補完了。90-1 ~ 114-2 共 50 個學期全數收合**，115-1 是當期。

**`backfill.yml` 的排程因此在 2026-09-09 拿掉了，只剩手動觸發。** 補完之後
它每 6 小時醒來一次都只是空轉：守門讓後面每一步都 skip，對學校 0 個請求，
而且連 `runs.json` 都不會留紀錄（`main()` 在「沒有需要更新的學年期」時直接
return，不寫 run summary）——看不到、又一直在跑。

> 收合**不依賴這支**。115-1 以後變成歷史學期時，做收合的是 `syllabus.yml`：
> `select_semesters()` 永遠抓最新學期 + 超過 24 小時沒更新的，而首頁固定掛
> 兩個學期，所以 115-2 出現之後 115-1 還在首頁上、還是會被抓到，那時
> `is_frozen_semester(115, 1)` 為真就收合了。
